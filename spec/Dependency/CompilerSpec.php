...
class Dependency_CompilerSpec extends ObjectBehavior
{
    ...
    {
        $this->subject->shouldHaveType(Dependency_Compiler::class);
    }
    ...
    function it_declares_correct_getter_phpdoc_return_type_for_service_provided_by_static_factory_with_phpdoc()
    {
        ...
        expect($class)->toDeclareMethodReturnType('get_array', ArrayObject::class);
    }
        ...
        try {
            $this->given_compiled_to_unique_class(
                Dependency_Definition_List::factory()
                    ->from_array(
                        [
                            'undefined' => ['_settings' => ['class' => __NAMESPACE__.'\\UndefinedClass']],
                        ]
                    )
            );
            throw new FailureException("Expected exception, none thrown");
        ...
        try {
            $this->given_compiled_to_unique_class(
                Dependency_Definition_List::factory()
                    ->from_array(
                        [
                            'invalid' => ['_settings' => ['class' => '\DateTime', 'arguments' => ['%undefined%']]],
                        ]
                    )
            );
            throw new FailureException("Expected exception, none thrown");
